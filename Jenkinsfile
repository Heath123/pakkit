    ...
    stages {
        stage('build') {
            steps {
                ...
                sh 'cd out/pakkit-win32-x64/resources/app/node_modules; node-prune; cd java; node postInstall.js'
                sh 'npx electron-forge package --platform linux'
                sh 'cd out/pakkit-linux-x64/resources/app/node_modules; node-prune; cd java; node postInstall.js'
                sh 'npx electron-forge package --platform darwin'
                 sh 'cd out/pakkit-darwin-x64/resources/app/node_modules; node-prune; cd java; node postInstall.js'
